...
const parseSeriesLinks = (html) => [...html.matchAll(/<a[^>]+href="(?:https?:\/\/www\.bbc\.co\.uk)?(\/programmes\/[a-z0-9]+\/episodes\/guide)"[^>]*>([\s\S]{0,2000}?)<\/a>/gi)].map((match) => ({ url: `https://www.bbc.co.uk${match[1]}`, label: clean(match[2]), series: Number(clean(match[2]).match(/series\s*(\d+)/i)?.[1] || 0) })).filter((series) => series.series);
...
for (let page = 1; page <= 4; page += 1) { const guidePageUrl = page === 1 ? guideUrl : `${guideUrl}?page=${page}`; const html = await fetchHtml(guidePageUrl); const seriesLinks = parseSeriesLinks(html); const resolvedSeriesLinks = seriesLinks.length ? seriesLinks : parseSeriesLinksLoose(html); console.log(`Guide page ${page}: ${html.length} bytes, ${resolvedSeriesLinks.length} series links`); if (!resolvedSeriesLinks.length) { console.warn(`No series links found. First programme-guide URLs: ${(html.match(/\/programmes\/[a-z0-9]+\/episodes\/guide/gi) || []).slice(0, 3).join(', ') || 'none'}`); } for (const series of resolvedSeriesLinks) { const seriesHtml = await fetchHtml(series.url); const seriesEpisodes = parseEpisodes(seriesHtml, series.series); console.log(`  ${series.label.slice(0, 50)}: ${seriesEpisodes.length} episode links`); for (const episode of seriesEpisodes) episodesByPid.set(episode.pid, episode); } console.log(`Guide page ${page}: ${episodesByPid.size} episode links collected`); }
