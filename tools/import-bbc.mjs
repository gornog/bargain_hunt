...
const fetchHtml = async (url) => { const response = await fetch(url, { headers }); const body = await response.text(); if (!response.ok) throw new Error(`${response.status} ${url} (${body.length} bytes)`); return body.replaceAll("'", '"'); };
